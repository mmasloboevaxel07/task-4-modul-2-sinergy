...
public class HomeWork4 {
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (String s : stringList) {
            boolean isHaveNotCharsInUpperCase = true;
            for (int i = 0; i < s.length(); i++) {
                if (Character.isUpperCase(s.charAt(i))) isHaveNotCharsInUpperCase = false;
            }
            if (isHaveNotCharsInUpperCase) correctedList.add(s);
        }
        return correctedList;
    }
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        ConcurrentHashMap<String, Integer> shopCartConcurrent = new ConcurrentHashMap<>(shopCart);
        for (String s : shopCartConcurrent.keySet()) if (shopCartConcurrent.get(s) > 300) shopCartConcurrent.remove(s);
        shopCart.clear();
        shopCart.putAll(shopCartConcurrent);
        return shopCart;
    }
